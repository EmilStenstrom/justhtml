...
- (Severity: Moderate) Harden custom foreign-namespace policies against additional active SVG elements. Previously, preserved SVG elements such as `<script>`, `<handler>`, `<animateColor>`, `<animateMotion>`, `<animateTransform>`, `<discard>`, and `<mpath>` could survive custom policies that allowed foreign content.
